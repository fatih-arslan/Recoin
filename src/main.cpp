...
#if defined(NDEBUG)
# error "Recoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x74ef5575ac9486ecbacd7eba5cb68a3204477f9a3b0c6805d43a8a2cc1688a09");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Recoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Recoin Signed Message:\n";
...
{
    // Recoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 420000); // Recoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 3 * 24 * 60 * 60; // Recoin: 3 days
static const int64 nTargetSpacing = 2 * 60; // Recoin: 2 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Recoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Recoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xb7e112f7f4403a259bf0adef1dd4340d5a845b082bb0eebb5f4bbd77cb4abfe4");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "2022/11/23";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 25 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("0483761f9693713778631893bf87325a4f1cd4e42714f2af74e1b0b348c05d9c38a8d137706c340d1175c7317f5756c1473284698e7da380ddfc7f6f15cab1882f") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1669230534;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2084817229;

        if (fTestNet)
        {
            block.nTime    = 1669230514;
            block.nNonce   = 386286810;
        }
	...
	if (true && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0xe6d574c5955f2188516357f48938d274f3d70cfc50e50f02119faff7a19940a7"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfc, 0xc1, 0xb7, 0xdc }; // Recoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("RecoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("RecoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static RecoinMiner(CWallet *pwallet)
{
    printf("RecoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("recoin-miner");
    ...
    try { loop {
        ...
        printf("Running RecoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&RecoinMiner, pwallet));
}
